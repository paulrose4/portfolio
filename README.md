# AI Agent Engineering Portfolio

Recruiting-oriented portfolio deployed with GitHub Pages.

## Featured project

- [SupportOS](https://github.com/paulrose4/obsidian-rag-support-agent): company internal AI customer-support platform for which I independently owned requirements analysis, architecture, full-stack development, deployment and maintenance. It connects about 10 multi-category commerce sites managed across 6 business workspaces.
- The SupportOS repository contains real business-project source code published with explicit company authorization; customer PII, secrets and private operational data remain excluded.
- [Live product login](https://livechatgo.com/)
- [Portfolio site](https://paulrose4.github.io/portfolio/)

## Local preview

The site uses static HTML, CSS and JavaScript. Open `index.html` directly or run any static file server.

## Public evidence policy

- Project metrics must be reproducible from source code, tests or versioned reports.
- Offline evaluation results are not presented as online accuracy.
- PostgreSQL/Qdrant/Redis integration tests are described separately from the default test baseline.
- Stale or environment-unbound evaluation artifacts are not shown as current passing evidence.
- Screenshots must not expose sensitive vertical-business content, customer PII, secrets or private operational data.
